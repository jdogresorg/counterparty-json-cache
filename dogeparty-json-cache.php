...
$dbase    = 'XChain'; // Name of the database where dogeparty_nfts table exists

// Load config (only after runtype is defined)
require_once(__DIR__ . '/includes/config.php');

// Define some constants used for locking processes and logging errors
define("LOCKFILE", '/var/tmp/dogeparty-json-cache-' . $runtype . '.lock');
define("LASTFILE", '/var/tmp/dogeparty-json-cache-' . $runtype . '.last-block');
define("ERRORLOG", '/var/tmp/dogeparty-json-cache-' . $runtype . '.errors');
        ...
        if(preg_match('/.json$/i', $row->description)){
            ...
            if($data){
                ...
                if($json){
                    ...
                    $results2 = $mysqli->query("SELECT id, hash FROM {$dbase}.dogeparty_json WHERE asset_id='{$row->id}' LIMIT 1");
                    if($results2){
                        if($results2->num_rows==0){
                            $sql = "INSERT INTO {$dbase}.dogeparty_json (testnet, asset_id, url, json, hash, created, updated) values ({$testnet}, {$row->id}, '{$url}','{$json}', '{$hash}',now(), now() )";
                        } else {
                            $info = (object) $results2->fetch_assoc();
                            // Only update if the sha256 hashes differ
                            if($info->hash != $hash)
                                $sql = "UPDATE {$dbase}.dogeparty_json SET url='{$url}', json='{$json}', hash='{$hash}', updated=now() WHERE id='{$info->id}'";
                        }
                    }
                    // Handle creating / Updating the record in dogeparty_json
                    if($sql){
